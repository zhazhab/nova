...
func TestSSEWriteHandlerAppliesMiddlewareChainBeforeWrite(t *testing.T) {
	...
	if count := strings.Count(got, "event: tool_args_delta"); count != 1 {
		t.Fatalf("tool_args_delta events = %d, want 1; output=%q", count, got)
	}
}
