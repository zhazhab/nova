...
Nova 可以部署在自己的服务器上，从手机浏览器访问前端。

### 1. 准备 Web 资源

Release 压缩包已附带构建好的 `web/` 目录，与 `nova` 二进制放在同一目录即可直接使用。从源码自托管时先构建前端：

```bash
pnpm --dir web build      # 产物输出到 web/dist
```

二进制启动时按顺序查找 Web 资源目录：`NOVA_WEB_DIR` 环境变量 → 可执行文件同级的 `web/` → 当前目录的 `web/`，可用 `NOVA_WEB_DIR=/path/to/web/dist` 显式指定。

### 2. 开启远程访问

在 Nova 的 **设置页 → 远程访问** 中开启「允许局域网访问」并设置用户名和密码（密码以 bcrypt 哈希存储）。开启后：

- 服务监听地址从 `127.0.0.1` 切换为 `0.0.0.0`，局域网/外网设备可连接；
- 所有非本机请求都需要 Basic 认证（用户名 + 密码），本机请求不受影响；
- 手机浏览器首次打开会弹出登录浮层，输入凭据后即可使用。

后端端口在 `config.toml` 的 `backend_port`（默认 8080）。启动时控制台会打印局域网地址，例如 `http://192.168.x.x:8080`。

### 3. 手机上使用

在手机浏览器打开上面的地址登录后，可以「添加到主屏幕」，以独立应用形态（无浏览器地址栏、适配刘海安全区）打开。Nova 已提供 PWA manifest 与应用图标。

### 4. HTTPS / 公网访问（推荐）

局域网内的明文 HTTP 足够个人使用；若要通过公网或域名访问，建议在前端加一层反向代理（Caddy / Nginx）提供 HTTPS：
